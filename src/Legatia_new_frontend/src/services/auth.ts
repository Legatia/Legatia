...
const DFX_NETWORK = (typeof process !== 'undefined' && process.env?.DFX_NETWORK) || 
                   (typeof window !== 'undefined' && (window as any).DFX_NETWORK) || 
                   'local';
const INTERNET_IDENTITY_URL = DFX_NETWORK === 'local' 
...
class AuthService {
  ...
  private async setupActor(): Promise<void> {
    ...
    if (DFX_NETWORK === 'local') {
      // Always use localhost:4943 for local development
    ...
    this.actor = createActor(
      (typeof process !== 'undefined' && (process.env as any).CANISTER_ID_LEGATIA_NEW_BACKEND) ||
      (typeof window !== 'undefined' && (window as any).CANISTER_ID_LEGATIA_NEW_BACKEND) ||
  ...
  private async setupDevActor(): Promise<void> {
    ...
    console.log('Setting up dev actor with canister ID:', 
      (typeof process !== 'undefined' && (process.env as any).CANISTER_ID_LEGATIA_NEW_BACKEND) ||
      (typeof window !== 'undefined' && (window as any).CANISTER_ID_LEGATIA_NEW_BACKEND) ||
      'uxrrr-q7777-77774-qaaaq-cai');
    console.log('Using authenticated principal for dev mode');
    
    // Use localhost:4943 for local development
    const agentHost = 'http://localhost:4943';
  ...
  private async setupMockActor(identity: Identity): Promise<void> {
    console.log('Setting up mock actor with canister ID:', 
      (typeof process !== 'undefined' && (process.env as any).CANISTER_ID_LEGATIA_NEW_BACKEND) ||
      (typeof window !== 'undefined' && (window as any).CANISTER_ID_LEGATIA_NEW_BACKEND) ||
      'uxrrr-q7777-77774-qaaaq-cai');
    
    // For local development, always use localhost:4943 which is the replica
    const agentHost = 'http://localhost:4943';
